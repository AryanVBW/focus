...
import com.aryanvbw.focus.detection.VideoContentDetector
import com.aryanvbw.focus.detection.ContentDetectionCoordinator
import com.aryanvbw.focus.blocking.ScrollBlockingHandler
import com.aryanvbw.focus.blocking.EnhancedBlockingManager
...
class FocusAccessibilityService : AccessibilityService() {
    ...
    // Enhanced detection system components
    private lateinit var videoContentDetector: VideoContentDetector
    private lateinit var contentDetectionCoordinator: ContentDetectionCoordinator
    private lateinit var scrollBlockingHandler: ScrollBlockingHandler
    private lateinit var enhancedBlockingManager: EnhancedBlockingManager
    ...
    override fun onCreate() {
        ...
        try {
            ...
            videoContentDetector = VideoContentDetector(appSettings)
            scrollBlockingHandler = ScrollBlockingHandler(this, appSettings)
            enhancedBlockingManager = EnhancedBlockingManager(this, this, appSettings)
            contentDetectionCoordinator = ContentDetectionCoordinator(
                legacyDetector = contentDetector,
                videoDetector = videoContentDetector,
                scrollBlockingHandler = scrollBlockingHandler,
    ...
    private fun initializeComponents() {
        ...
        // Initialize enhanced detection system
        if (!::videoContentDetector.isInitialized) {
            videoContentDetector = VideoContentDetector(appSettings)
        }
        
        if (!::scrollBlockingHandler.isInitialized) {
            scrollBlockingHandler = ScrollBlockingHandler(this, appSettings)
        }
        
        if (!::enhancedBlockingManager.isInitialized) {
            enhancedBlockingManager = EnhancedBlockingManager(this, this, appSettings)
        }
        
        if (!::contentDetectionCoordinator.isInitialized) {
            contentDetectionCoordinator = ContentDetectionCoordinator(
                legacyDetector = contentDetector,
                videoDetector = videoContentDetector,
                scrollBlockingHandler = scrollBlockingHandler,
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent) {
        ...
        try {
            ...
            if (!::appSettings.isInitialized || !::contentDetectionCoordinator.isInitialized) {
            ...
            if (event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED ||
                ...
                if (appSettings.isAppMonitored(packageName)) {
                    Log.d(TAG, "Processing accessibility tree for $packageName with enhanced detection")
                    
                    // Use the enhanced detection coordinator
                    val detectionResult = contentDetectionCoordinator.detectContent(rootNode, packageName)
                    ...
                    if (detectionResult.shouldBlock && appSettings.shouldBlockContent(packageName, detectionResult.contentType)) {
                        Log.i(TAG, "Enhanced detection: Blocking ${detectionResult.contentType} in $packageName using ${detectionResult.blockingStrategy} (Focus Mode: ${appSettings.isFocusModeEnabled()})")
                        
                        // Check if temporarily unblocked
                        if (appSettings.isTemporarilyUnblocked(packageName)) {
                            Log.d(TAG, "Package $packageName is temporarily unblocked, skipping blocking")
                            return
                        }
                        
                        // Execute enhanced blocking with all strategies
                        enhancedBlockingManager.executeBlocking(
                            packageName,
                            rootNode,
                            detectionResult.contentType
                        )
                        
                        // Also execute the original blocking strategy as fallback
                        val wasBlocked = contentDetectionCoordinator.executeBlocking(
                        ...
                        if (wasBlocked) {
                            ...
                            // Log the blocked content event
                            logBlockedContentEvent(packageName, detectionResult.contentType, detectionResult.detectionMethod.name)
                            
                            // Show notification if enabled
                            if (appSettings.showBlockNotifications()) {
                                notificationHelper.showContentBlockedNotification(
                                    packageName,
                                    detectionResult.contentType
                                )
                            }
            ...
            // --- Legacy: Handle full app blocking for blocked apps ---
            if (appSettings.isAppBlocked(packageName)) {
                Log.i(TAG, "Blocking entire app: $packageName")
                
                // Check if temporarily unblocked
                if (appSettings.isTemporarilyUnblocked(packageName)) {
                    Log.d(TAG, "Package $packageName is temporarily unblocked, skipping full app blocking")
                    return
                }
                
                // Use enhanced blocking for full app blocking
                enhancedBlockingManager.executeBlocking(packageName, rootNode, "blocked_app")
                
                // Also execute legacy blocking as fallback
            ...
            if (urlText != null) {
                ...
                for (keyword in ADULT_KEYWORDS) {
                    if (urlText.contains(keyword)) {
                        ...
                        logBlockedContentEvent(packageName, "adult_content", "URL_KEYWORD_MATCH")
                        return true // Blocked
                    }
                }
                ...
                if (urlHost != null) {
                    for (domain in ADULT_DOMAINS) {
                        if (urlHost.contains(domain)) { // contains checks subdomains too (e.g., m.xnxx.com)
                            ...
                            // Log the blocked event
                            logBlockedContentEvent(packageName, "adult_content", "URL_DOMAIN_MATCH")
                            return true // Blocked
                        }
                    }
                }
    ...
    override fun onDestroy() {
        ...
        // Cleanup enhanced blocking manager
        if (::enhancedBlockingManager.isInitialized) {
            enhancedBlockingManager.cleanup()
        }
    ...
    /**
     * Helper method to log blocked content events to the database
     */
    private fun logBlockedContentEvent(packageName: String, contentType: String, detectionMethod: String) {
        try {
            if (::blockedContentEventDao.isInitialized) {
                serviceScope.launch {
                    try {
                        ...
                        blockedContentEventDao.insert(event)
                        Log.d(TAG, "Logged blocked content event: $packageName - $contentType")
                    } catch (e: Exception) {
                        Log.e(TAG, "Error logging blocked content event: ${e.message}")
                    }
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error in logBlockedContentEvent: ${e.message}")
        }
    }
    
    /**
     * Helper method to get a user-friendly app name from package name
     */
